...
async function main() {
  console.log('Updating car images with Wikipedia Commons photos (using axios)...\n');
  ...
  for (const car of cars) {
    ...
    if (!imgUrl) {
      console.log(`⚠ No image mapping: ${car.make} ${car.model}`);
      failed++;
      continue;
    }
    ...
    try {
      ...
      const blob = await put(`cars/${car.vin.toLowerCase()}-wiki.jpg`, buffer, {
        access: 'public',
